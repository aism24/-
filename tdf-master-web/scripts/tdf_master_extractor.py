...
def determine_length_no_size(tdf: tb.TdfData, x_min: float, x_max: float, y_ref: float):
    ...
    サブ寸法和で一意に(タイ無しで)決まった候補が無い場合はNoneを返す
    (タイブレークは実装しない。ユーザー確認済み方針)。
    ...
    if not match_counts:
        return None, {**debug, "reason": "no sub-sum match"}

    max_c = max(match_counts.values())
    top = [v for v, c in match_counts.items() if c == max_c]
    if len(top) != 1:
        return None, {**debug, "reason": "tie among top candidates", "top": top}
...
def determine_joints_no_size(tdf: tb.TdfData, y_ref: float, length_value: float, endpoints):
    ...
    の近傍にある長丸のうち、各端点に最も近いものを左右継手候補とする
    (絶対閾値は設けない)。

    この図面では長丸の中心ぴったりにテキストが無く、少し離れた位置に
    継手コードがあったため、テキスト自体も「最も近いもの」を絶対閾値なしで
    採用する(ユーザー確認済み方針)。
    ...
    centers = [c for c in find_stadium_centers(tdf) if c[1] > y_ref]
    if not centers:
        return None, None, None, None

    left_center = min(centers, key=lambda c: math.hypot(c[0] - lx, c[1] - ly))
    right_center = min(centers, key=lambda c: math.hypot(c[0] - rx, c[1] - ry))

    def _nearest_text(cx: float, cy: float):
        best = None
        best_d = float("inf")
        for r in tdf.texts:
            if r.rot != 0:
    ...
    left_text = _nearest_text(left_center[0], left_center[1])
    right_text = _nearest_text(right_center[0], right_center[1])
    return (
        left_text, right_text,
        {"cx": left_center[0], "cy": left_center[1]},
        {"cx": right_center[0], "cy": right_center[1]},
    )
...
def determine_size_no_size(tdf: tb.TdfData, endpoints, left_jobj, right_jobj):
    ...
    そのY座標帯(±300)・長さのX範囲内で改めて単一候補を探す。

    候補が0件または複数件(値が割れる)の場合はNoneを返す(フォールバックは
    実装しない。ユーザー確認済み方針)。
    """
    if endpoints is None:
        return None
