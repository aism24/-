 ...
 *     オペレータ・記録者列(任意の位置に見出しを追加): ロボット溶接フォームの「オペレータ」
 *            「記録者」プルダウンの選択肢(列名でマッチングするため位置は自由。半自動溶接の
 *            検査員別ゲート機構とは無関係の、他のマスタ列と同じ独立したフラットなリスト)。
...
const MASTER_COLUMNS = ["工事名", "溶接者", "検査員", "部材", "材質", "溶接方法", "オペレータ", "記録者"];
